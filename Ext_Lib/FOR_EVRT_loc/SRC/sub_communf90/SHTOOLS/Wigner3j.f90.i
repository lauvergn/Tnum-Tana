...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    real (kind=Rkind), intent(out) ::  w3j(:)
    real (kind=Rkind) ::       wnmid, wpmid, scalef, denom, rs(j2+j3+1), &
    ...
    if (jnum == 1) then
        w3j(1) = ONE / sqrt( real(2*jmin+1,kind=Rkind) )
        if ( (w3j(1) < ZERO .and. (-1)**(j2-j3+m2+m3) > 0) .or. &
            (w3j(1) > ZERO .and. (-1)**(j2-j3+m2+m3) < 0) ) &
            w3j(1) = -w3j(1)
        return
    endif
    ...
    if (m1 == 0 .and. m2 == 0 .and. m3 == 0) then       ! All m's are zero
        ...
        wl(jindex(jmin)) = ONE
        wl(jindex(jmin+1)) = ZERO
        jn = jmin+1

    elseif (yjmin == ZERO) then                ! The second terms is either zero

        if (xjmin == ZERO) then            ! or undefined
            flag1 = 1
            jn = jmin
        else
            wl(jindex(jmin)) = ONE
    ...
    elseif ( xjmin * yjmin >= ZERO) then           ! The second term is outside of the
        ...
        do j=jmin + 1, jmax-1, 1
            ...
            xj = x(j)
            if (abs(xj) > abs(denom) .or. xj * denom >= ZERO .or. denom == ZERO) then
                jn = j-1
                exit
            else
    ...
    if (m1 == 0 .and. m2 == 0 .and. m3 == 0) then
        ...
        wu(jindex(jmax)) = ONE
        wu(jindex(jmax-1)) = ZERO
        jp = jmax-1

    elseif (yjmax == ZERO) then
    ...
    elseif (yjmax * zjmax >= ZERO) then
        ...
        do j=jmax-1, jn, -1
            ...
            zj = z(j)
            if (abs(zj) > abs(denom) .or. zj * denom >= ZERO .or. denom == ZERO) then
                jp = j+1
                exit
            else
    ...
    if (flag1 == 0) then
        ...
        do j=jn, jmid - 1, 1
            ...
            if (abs(wl(jindex(j+1))) > ONE) then              ! watch out for overflows.
                wl(jindex(jmin):jindex(j+1)) = wl(jindex(jmin):jindex(j+1)) / scalef
            endif

            if (abs(wl(jindex(j+1)) / wl(jindex(j-1))) < ONE .and. &  ! if values are decreasing
                wl(jindex(j+1)) /= ZERO) then              ! then stop upward iteration
        ...
        if (abs(wnmid/wl(jindex(jmid-1))) < 1.d-6 .and. &
            wl(jindex(jmid-1)) /= ZERO) then               ! Make sure that the stopping
    ...
    contains
        ...
        real (kind=Rkind) function a(j)
            integer :: j
            a = (real(j,kind=Rkind)**2 - real(j2-j3,kind=Rkind)**2) *   &
                (real(j2+j3+1,kind=Rkind)**2 - real(j,kind=Rkind)**2) * &
                (real(j,kind=Rkind)**2-real(m1,kind=Rkind)**2)
            a = sqrt(a)
        end function a

        real (kind=Rkind) function y(j)
            integer :: j
            y = -real(2*j+1,kind=Rkind) * &
                ( real(m1,kind=Rkind) * (real(j2,kind=Rkind)*real(j2+1,kind=Rkind) - &
                real(j3,kind=Rkind)*real(j3+1,kind=Rkind) ) - real(m3-m2,kind=Rkind)*&
                real(j,kind=Rkind)*real(j+1,kind=Rkind) )
        end function y

        real (kind=Rkind) function x(j)
            integer :: j
            x = real(j,kind=Rkind) * a(j+1)
        end function x

        real (kind=Rkind) function z(j)
            integer :: j
            z = real(j+1,kind=Rkind)*a(j)
        end function z
        ...
        subroutine normw3j
            ...
            norm = ZERO
            do j = jmin, jmax
                norm = norm + real(2*j+1,kind=Rkind) * w3j(jindex(j))**2
            enddo
        ...
        subroutine fixsign
            ...
            if ( (w3j(jindex(jmax)) < ZERO .and. (-1)**(j2-j3+m2+m3) > 0) .or. &
                (w3j(jindex(jmax)) > ZERO .and. (-1)**(j2-j3+m2+m3) < 0) ) then
