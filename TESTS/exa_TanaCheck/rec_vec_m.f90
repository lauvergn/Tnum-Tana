...
RECURSIVE SUBROUTINE recBF2_TO_recBF1(BF1,BF2)
  IMPLICIT NONE
  TYPE(recBF_t), intent(in) :: BF2
  TYPE(recBF_t), intent(inout) :: BF1
...
RECURSIVE SUBROUTINE Split_blocks_new(nv)
  ...
  TYPE(recBF_t) :: BF_vec_poly
  TYPE(recBF_t) :: BF_vec_cart
  TYPE(recBF_t) :: BF_vec_frame
  TYPE(recBF_t) :: BF_vec_xzy

  TYPE(recBF_t) :: BF_vec(nv)
  ...
  BF_vec(1)%tab_BF(1)%vec_name = " &vector /"  ! it is not going to be used
  ...
  DO iv=2,max_v
    ...
    DO
      ...
      DO ! because in BF_vec(iiv) there are several BF (in tab_BF)
        ...
        IF (zmat_order) THEN
          BF_vec(iv)%tab_BF(i_BF)%vec_name = &
  ...
  DO i_BF=1,size(BF_vec(iv)%tab_BF)
    ...
    data_name = "dat_nv" // TO_string(iv) // "-num" // TO_string(i_BF)
